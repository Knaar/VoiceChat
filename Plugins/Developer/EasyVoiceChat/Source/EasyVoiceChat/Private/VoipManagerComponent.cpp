...
TArray<FString> UVoipManagerComponent::GetCaptureDevices()
{
	TArray<FString> OutDevices;

	// This is exactly like the existing blueprint function, however it doesn't filter the results
...
bool UVoipManagerComponent::ChangeAudioCaptureDevice(FString NewDeviceName)
{
	if (VoiceCapture.IsValid())
	{
		VoiceCapture.Get()->ChangeDevice(NewDeviceName, UVOIPStatics::GetVoiceSampleRate(), UVOIPStatics::GetVoiceNumChannels());
		return VoiceCapture.Get()->Start();
	}
